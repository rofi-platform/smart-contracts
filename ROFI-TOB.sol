...
// ROFITOB with Governance.
contract ROFITOB is BEP20('ROFITOB', 'ROFITOB'), Pausable, IROFITOB, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    modifier onlyAuthorizedUnlockedMintCaller() {
        require(_msgSender() == owner() || _authorizedUnlockedMintCaller[_msgSender()],"ROFITOBP: UNLOCKED_MINT_CALLER_NOT_AUTHORIZED");
        _;
    }
    
    modifier onlyAuthorizedLockedMintCaller() {
        require(_msgSender() == owner() || _authorizedUnlockedMintCaller[_msgSender()] || _authorizedLockedMintCaller[_msgSender()],"ROFITOBP: LOCKED_MINT_CALLER_NOT_AUTHORIZED");
        _;
    }
    ...
    function stake(address token, uint256 amount) external override nonReentrant returns (bool) {
        require(_unlockFactor[token] > 0, "ROFITOB: FACTOR_NOT_SET");
        require(_unlockBlockGap[token] > 0, "ROFITOB: BLOCK_GAP_NOT_SET");
    ...
    function unstake(address token, uint256 amount) external override nonReentrant returns (bool) {
        require(amount > 0, "ROFITOB: ZERO_UNSTAKE_AMOUNT");
        LpStakeInfo storage info = _stakingRecords[_msgSender()][token];
        require(amount <= info.amountStaked, "ROFITOB: UNSTAKE_AMOUNT_EXCEEDED");
        uint256 unlockedAmount = _settleUnlockAmount(_msgSender(), token, info.amountStaked, info.blockNumber);
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "ROFITOB::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "ROFITOB::delegateBySig: invalid nonce");
        require(now <= expiry, "ROFITOB::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "ROFITOB::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying ROFITOBs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "ROFITOB::_writeCheckpoint: block number exceeds 32 bits");
