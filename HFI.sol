...
contract HFIToken is BEP20('HFITOB', 'HFITOB'), Pausable, ITOKEN, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    modifier onlyAuthorizedUnlockedMintCaller() {
        require(_msgSender() == owner() || _authorizedUnlockedMintCaller[_msgSender()],"HFI: UNLOCKED_MINT_CALLER_NOT_AUTHORIZED");
        _;
    }
    
    modifier onlyAuthorizedLockedMintCaller() {
        require(_msgSender() == owner() || _authorizedUnlockedMintCaller[_msgSender()] || _authorizedLockedMintCaller[_msgSender()],"HFI: LOCKED_MINT_CALLER_NOT_AUTHORIZED");
        _;
    }
    ...
    constructor() public {
        _maxCap = 1000000000 * (10 ** uint256(decimals()));
    }    
    ...
    function stake(address token, uint256 amount) external override nonReentrant returns (bool) {
        require(_unlockFactor[token] > 0, "HFI: FACTOR_NOT_SET");
        require(_unlockBlockGap[token] > 0, "HFI: BLOCK_GAP_NOT_SET");
    ...
    function unstake(address token, uint256 amount) external override nonReentrant returns (bool) {
        require(amount > 0, "HFI: ZERO_UNSTAKE_AMOUNT");
        LpStakeInfo storage info = _stakingRecords[_msgSender()][token];
        require(amount <= info.amountStaked, "HFI: UNSTAKE_AMOUNT_EXCEEDED");
        uint256 unlockedAmount = _settleUnlockAmount(_msgSender(), token, info.amountStaked, info.blockNumber);
    ...
    function mintUnlockedToken(address to, uint256 amount) onlyAuthorizedUnlockedMintCaller external override {
        require(amount.add(totalSupply()) <= _maxCap, "HFI: MAX_CAP_REACHED");
        _mint(to, amount);
