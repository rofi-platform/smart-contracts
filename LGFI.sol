...
// LGFIToken with Governance.
contract LGFIToken is BEP20('LGFI', 'LGFI'), Pausable, ITOKEN, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    modifier onlyAuthorizedUnlockedMintCaller() {
        require(_msgSender() == owner() || _authorizedUnlockedMintCaller[_msgSender()],"LGFI: UNLOCKED_MINT_CALLER_NOT_AUTHORIZED");
        _;
    }
    
    modifier onlyAuthorizedLockedMintCaller() {
        require(_msgSender() == owner() || _authorizedUnlockedMintCaller[_msgSender()] || _authorizedLockedMintCaller[_msgSender()],"LGFI: LOCKED_MINT_CALLER_NOT_AUTHORIZED");
        _;
    }
    ...
    constructor() public {
        _maxCap = 100000000000 * (10 ** uint256(decimals()));
    }    
    ...
    function stake(address token, uint256 amount) external override nonReentrant returns (bool) {
        require(_unlockFactor[token] > 0, "LGFI: FACTOR_NOT_SET");
        require(_unlockBlockGap[token] > 0, "LGFI: BLOCK_GAP_NOT_SET");
    ...
    function unstake(address token, uint256 amount) external override nonReentrant returns (bool) {
        require(amount > 0, "LGFI: ZERO_UNSTAKE_AMOUNT");
        LpStakeInfo storage info = _stakingRecords[_msgSender()][token];
        require(amount <= info.amountStaked, "LGFI: UNSTAKE_AMOUNT_EXCEEDED");
        uint256 unlockedAmount = _settleUnlockAmount(_msgSender(), token, info.amountStaked, info.blockNumber);
    ...
    function mintUnlockedToken(address to, uint256 amount) onlyAuthorizedUnlockedMintCaller external override {
        require(amount.add(totalSupply()) <= _maxCap, "LGFI: MAX_CAP_REACHED");
        _mint(to, amount);
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "LGFI:delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "LGFI:delegateBySig: invalid nonce");
        require(now <= expiry, "LGFI:delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "LGFI:getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "LGFI:_writeCheckpoint: block number exceeds 32 bits");
