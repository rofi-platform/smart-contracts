...
contract LimitChest is Ownable {
    ...
    function updateChest(uint8 _id, uint256 _price, uint8[] memory _percents, uint256 _totalSupply, address _paymentToken, bool _useChainlink) external onlyOwner {
        uint256 length = _percents.length;
    ...
    function getChestOpenedAmount(uint8 _id) public view returns (uint256) {
        return records[_id];
    }

    function getTotalChestCanOpen(uint8 _id) public view returns (uint256) {
        ChestType memory chest = chests[_id];
        return chest.totalSupply;
    }

    function getRemainingChestAmount(uint8 _id) public view returns (uint256) {
        ChestType memory chest = chests[_id];
        return chest.totalSupply.sub(records[_id]);
    }
