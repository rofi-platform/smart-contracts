...
interface Item {
    ...
    function mintItem(address _to, uint8 _star, uint8 _itemType) external;

    function getItemTypes(uint8 _star) external returns (uint8[] memory);
}
...
contract Chest is Ownable {
    ...
    function openChest(uint8 _id, uint8 _amount) external {
        ...
        if (chest.useChainlink) {
            requestRandomNumber(_id, _amount, block.timestamp);
        } else {
            uint8 randomNumber = uint8(getRandomNumber().mod(100).add(1));
            _openChest(_id, randomNumber, _amount, false, block.timestamp);
        }
    }

    function _openChest(uint8 _id, uint8 _randomNumber, uint8 _amount, bool _useChainlink, uint256 _timestamp) internal {
        ChestType memory chest = chests[_id];
        ...
        while (time < _amount) {
            ...
            uint8[] memory itemTypes = item.getItemTypes(star);
            uint256 totalTypes = itemTypes.length;
            uint8 itemType = itemTypes[uint256(_randomNumber).mod(totalTypes)];
            item.mintItem(_msgSender(), star, itemType);
            uint256 newItemId = item.latestItemId();
            itemIds[time] = newItemId;
            time++;
        }
        emit ChestOpen(_msgSender(), _id, itemIds, _timestamp);
    }
    ...
    function requestRandomNumber(uint8 _id, uint8 _amount, uint256 _timestamp) internal {
    ...
    function submitRandomness(uint _requestId, uint _randomness) external onlyRandom {
        ...
        _openChest(chestId, uint8(_randomness), amount, true, timestamp);
    }
